...
def net_train(fold, net, train_loader, dev_iter):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (text_, video_, audio_, label_) in enumerate(train_loader):
            ...
            combine_out_adv = net(text_, video_, audio_)
            loss_adv = F.binary_cross_entropy(combine_out_adv, label_)
            ...
            if total_batch % 50 == 0:
                ...
                if dev_best_acc < dev_acc:
                    ...
                    torch.save(net, 'checkpoint/efficient_best_fgm_{}.pth'.format(fold))
...
def train_efficient_fgm():
    
    for i in range(10):
        print("Loading  Fold data {}".format(i))
        df_train = pd.read_csv('src/data/Fold_data/efficient_10/data_path_efficient_train_{}.csv'.format(i))
        df_dev = pd.read_csv('src/data/Fold_data/efficient_10/data_path_efficient_val_{}.csv'.format(i))
        ...
        unfreeze_layers = ['word_embeddings','layer.9','layer.10', 'layer.11', 'out.']
...
if __name__ == '__main__':
    ...
    for i in range(10):
        print("Loading  Fold data {}".format(i))
        df_train = pd.read_csv('./Fold_data/efficient_10/data_path_efficient_train_{}.csv'.format(i))
        df_dev = pd.read_csv('./Fold_data/efficient_10/data_path_efficient_val_{}.csv'.format(i))
        train_data = MuliDataset(df_train, device)
        dev_data = MuliDataset(df_dev, device)
