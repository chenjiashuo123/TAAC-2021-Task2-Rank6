...
def pre_func(model, out_file=''):
    ...
    df_train = pd.read_csv('src/data/Fold_data/data_path_test_2nd_400_vit.csv')
...
def predict_vit():
    for i in range(10):
        ...
        model.load_state_dict(torch.load('checkpoint/vit_best_{}.pth'.format(i)))
        model.eval()
        pre_func(model)
        with open('post-processing/inter_json/vit_{}.json'.format(i), 'w') as f:
            json.dump(res_to_write, f, ensure_ascii=False, indent = 4) 
            
if __name__ == '__main__':
    
    for i in range(10):
        model = torch.load('checkpoint/vit_best_{}.pth'.format(i)).cuda()
        pre_func(model)
        with open('post-processing/inter_json/vit_{}.json'.format(i), 'w') as f:
            json.dump(res_to_write, f, ensure_ascii=False, indent = 4) 
